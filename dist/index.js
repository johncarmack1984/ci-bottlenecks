...
function buildPositionMap(source) {
  ...
  if (!$isMap(doc.contents))
    return positions;
  const jobsItem = doc.contents.items.find((item) => $isScalar(item.key) && item.key.value === "jobs");
  if (!jobsItem || !$isMap(jobsItem.value))
    return positions;
  for (const jobItem of jobsItem.value.items) {
    if (!$isScalar(jobItem.key) || !jobItem.key.range)
      continue;
    const jobId = String(jobItem.key.value);
    positions.set(`jobs.${jobId}`, offsetToLine(lineOffsets, jobItem.key.range[0]));
    if (!$isMap(jobItem.value))
      continue;
    const stepsItem = jobItem.value.items.find((item) => $isScalar(item.key) && item.key.value === "steps");
    if (!stepsItem || !$isSeq(stepsItem.value))
      continue;
    for (let i = 0;i < stepsItem.value.items.length; i++) {
      ...
      if (stepNode && typeof stepNode === "object" && "range" in stepNode && stepNode.range) {
        positions.set(`jobs.${jobId}.steps[${i}]`, offsetToLine(lineOffsets, stepNode.range[0]));
      }
    }
  }
  return positions;
}
function parseSuppressionComment(text) {
  const match = text.match(/# ci-bottlenecks:\s*ignore(?:\[([^\]]+)\])?/);
  if (!match)
    return null;
  if (match[1])
...
function findSuppressionAbove(sourceLines, lineNum) {
...
function findSuppressionAt(sourceLines, lineNum) {
...
function findSuppressionInRange(sourceLines, startLine, endLine) {
...
function parseSuppressions(sourceLines, positions) {
  const result = {
    workflow: [],
    jobs: new Map,
    steps: new Map
  ...
  for (const [key, lineNum] of positions) {
    if (key.startsWith("jobs.") && !key.includes(".steps[")) {
      ...
      if (sup)
        result.jobs.set(key.slice(5), sup);
      ...
      if (sup)
        result.steps.set(`${jobId}:${stepIdx}`, sup);
    }
  }
  return result;
}
function parseTriggers(on) {
  if (typeof on === "string") {
    return { [on]: {} };
  }
  if (Array.isArray(on)) {
...
function parseJob(id, raw, positions) {
  const steps = [];
  ...
  const needs = [];
  if (typeof raw.needs === "string")
    needs.push(raw.needs);
  else if (Array.isArray(raw.needs))
    needs.push(...raw.needs.map(String));
  const runsOn = raw["runs-on"];
  let runsOnValue;
  if (typeof runsOn === "string")
    runsOnValue = runsOn;
  else if (Array.isArray(runsOn))
    runsOnValue = runsOn.map(String);
  else
    runsOnValue = JSON.stringify(runsOn ?? "");
  ...
  return {
    ...
    "runs-on": runsOnValue,
    needs,
    "timeout-minutes": raw["timeout-minutes"],
...
`);
  const name = raw.name ?? path.split("/").pop()?.replace(/\.ya?ml$/, "") ?? path;
  ...
  if (rawJobs && typeof rawJobs === "object") {
    for (const [jobId, jobDef] of Object.entries(rawJobs)) {
      if (!jobDef || typeof jobDef !== "object")
        continue;
      jobs.set(jobId, parseJob(jobId, jobDef, positions));
    }
  }
...
function parseActionRef(uses) {
  ...
  if (atIdx === -1)
    return { key: clean, version: "", isLocal: false };
...
function isSuppressed(finding, workflow) {
  ...
  if (sup.workflow === "all")
    return true;
  if (Array.isArray(sup.workflow) && sup.workflow.includes(finding.rule))
    return true;
  if (finding.job) {
...
function crossTierGate(findings, workflows, auditDataByWorkflow, pedantic) {
  ...
  for (const f of findings) {
    ...
    const summary2 = measured.map((m) => `${m.name}: ${fmtMinutes(m.medianMs)} median over ${m.samples} run${m.samples === 1 ? "" : "s"}`).join(", ");
    if (measured.every((m) => m.medianMs < INSTALL_CACHE_WORTH_MS)) {
      if (pedantic) {
        result.push({
          ...f,
          severity: "info",
          evidence: `Measured: ${summary2} — under the ${INSTALL_CACHE_WORTH_MS / 1000}s payback floor, a cache would save nothing`,
    ...
    result.push({
      ...f,
      severity: "medium",
      evidence: `Measured: ${summary2}. No matching cache action or configuration found`,
      remediation: cacheRemediation(eco),
...
function runRules(rules, workflows, options) {
  ...
  const activeRules = rules.filter((r) => {
    if (r.tier === "audit" && !options.audit)
      return false;
    if (r.pedantic && !options.pedantic)
      return false;
    return true;
  });
...
// src/rules/no-concurrency.ts
function hasCancelInProgress(concurrency) {
  if (!concurrency)
    return false;
  const cip = concurrency["cancel-in-progress"];
  if (cip === true)
    return true;
  if (typeof cip === "string" && cip.includes("${{"))
    return true;
  return false;
}
function hasDeliberateConcurrency(concurrency) {
  if (!concurrency)
    return false;
  if (concurrency.group && concurrency["cancel-in-progress"] === false)
    return true;
  return hasCancelInProgress(concurrency);
}
function isTagsOnlyPush(triggers) {
  const push = triggers.push;
...
var noConcurrency = {
  ...
  tier: "static",
  severity: "medium",
  describe: "Workflow triggered on push/PR with no concurrency + cancel-in-progress",
  check(ctx) {
    const { triggers } = ctx.workflow;
    const hasPushOrPR = triggers.push !== undefined || triggers.pull_request !== undefined || triggers.pull_request_target !== undefined;
    if (!hasPushOrPR)
      return [];
    if (isTagsOnlyPush(triggers) && !triggers.pull_request && !triggers.pull_request_target)
      return [];
    if (hasDeliberateConcurrency(ctx.workflow.concurrency))
      return [];
    for (const [, job] of ctx.workflow.jobs) {
      if (hasDeliberateConcurrency(job.concurrency))
        return [];
    }
    return [
      {
        ...
        tier: "static",
        workflow: ctx.workflow.path,
        message: "Workflow is triggered by push/PR but has no concurrency group with cancel-in-progress",
        evidence: "No concurrency block with cancel-in-progress: true at workflow or job level",
        remediation: "Add a concurrency block to cancel outdated runs when new commits are pushed.",
        patch: `concurrency:
...
// src/rules/cache-key-no-hash.ts
function cacheActionType(uses) {
  const { key } = parseActionRef(uses);
  if (key !== "actions/cache")
    return null;
  if (uses.includes("/cache/save"))
    return "save";
  if (uses.includes("/cache/restore"))
    return "restore";
  return "cache";
}
function hasHashOrExpression(value) {
      ...
      for (const step of job.steps) {
        ...
        if (!hasHashOrExpression(key)) {
          findings.push({
            ...
            remediation: "Add hashFiles() over your lockfile or dependency manifest to the cache key.",
          ...
          findings.push({
            ...
            remediation: "Add restore-keys so a partial cache hit can still speed up the build.",
            patch: `restore-keys: |
  \${{ runner.os }}-cache-`
          });
        }
      ...
      for (const manual of manualCacheSteps) {
        if (hasRustCache && RUST_CACHE_PATHS2.test(manual.paths)) {
          findings.push({
            ...
            remediation: "Remove the manual actions/cache step; Swatinem/rust-cache already handles cargo/target caching."
          });
        }
    ...
    for (const [jobId, job] of ctx.workflow.jobs) {
      for (const step of job.steps) {
        ...
        findings.push({
          ...
          remediation: "Remove fetch-depth: 0 (or set to 1) to speed up checkout. Re-add if a step genuinely needs history.",
...
// src/rules/macos-not-needed.ts
var MACOS_COMMANDS = /\b(xcodebuild|xcrun|codesign|notarytool|security|brew|swift|fastlane)\b/;
var MACOS_KEYWORDS = /\b(tauri\s+ios|tauri\s+macos|apple-|ios|darwin|mac|osx)\b/i;
...
function hasMacOSWork(job) {
  ...
  for (const step of job.steps) {
    const run = step.run ?? "";
    if (MACOS_COMMANDS.test(run) || MACOS_KEYWORDS.test(run) || MACOS_EXTENSIONS.test(run) || MACOS_RUN_PATTERNS.test(run))
      return true;
    if (step.uses) {
      if (MACOS_KEYWORDS.test(step.uses) || MACOS_ACTION_PATTERNS.test(step.uses))
        return true;
      const { key } = parseActionRef(step.uses);
      if (key === "actions/upload-artifact")
        return true;
      if (MACOS_ACTION_PATTERNS.test(key))
        return true;
    }
    if (step.with) {
      const withValues = Object.values(step.with).map(String).join(" ");
      if (MACOS_COMMANDS.test(withValues) || MACOS_KEYWORDS.test(withValues) || MACOS_EXTENSIONS.test(withValues) || MACOS_RUN_PATTERNS.test(withValues))
        return true;
    }
  }
  if (job.strategy?.matrix) {
    const matrixStr = JSON.stringify(job.strategy.matrix);
    if (/macos|darwin|ios|mac|osx/i.test(matrixStr))
      return true;
  }
  return false;
}
function isMacOSRunner(runsOn) {
...
function stringifyJobValues(job) {
  const parts = [];
  if (job.env)
    parts.push(JSON.stringify(job.env));
  if (job.outputs)
    parts.push(JSON.stringify(job.outputs));
  if (job.if)
    parts.push(job.if);
  if (job.name)
    parts.push(job.name);
  ...
  if (job.strategy)
    parts.push(JSON.stringify(job.strategy));
  if (job.environment)
    parts.push(JSON.stringify(job.environment));
  for (const step of job.steps) {
    if (step.run)
      parts.push(step.run);
    if (step.uses)
...
function expressionToPattern(name) {
  const escaped = name.replace(/\$\{\{[^}]*\}\}/g, "<<<EXPR>>>").replace(/[.*+?^${}()|[\]\\]/g, "\\$&").replace(/<<<EXPR>>>/g, ".*");
  return new RegExp(`^${escaped}$`);
}
function getArtifactNames(job, action) {
  const names = new Set;
  for (const step of job.steps) {
    if (!step.uses)
      continue;
...
function getDownloadPatterns(job) {
  const names = new Set;
  ...
  for (const step of job.steps) {
    ...
    if (key === "actions/download-artifact") {
      ...
      if (step.with?.["merge-multiple"])
        hasMergeMultiple = true;
      if (pattern)
...
function artifactHandoff(jobA, jobB) {
  ...
  if (uploads.size === 0)
    return false;
  if (downloads.size === 0 && patterns.size === 0 && !hasMergeMultiple)
    return false;
  if (uploads.has("") || downloads.has(""))
    return true;
  if (hasMergeMultiple)
    return true;
  ...
  for (const p of patterns) {
    ...
    for (const u of uploads) {
      ...
      if (globPattern.test(plain) || globPattern.test(u))
        return true;
    }
  ...
  check(ctx) {
    ...
    for (const [jobBId, jobB] of ctx.workflow.jobs) {
      ...
      for (const depId of jobB.needs) {
        ...
        if (bValues.includes(`needs.${depId}.outputs`))
          continue;
        if (bValues.includes(`needs.${depId}.result`))
          continue;
        if (/needs\.\*/.test(bValues))
          continue;
        if (/toJSON\(needs\)/.test(bValues))
          continue;
...
function getCargoFlags(job) {
  ...
  for (const step of job.steps) {
    ...
    if (targetMatch)
      flags.push(`target:${targetMatch[1]}`);
    const featMatch = step.run.match(/--features\s+(\S+)/);
    if (featMatch)
      flags.push(`features:${featMatch[1]}`);
  }
  return flags.sort().join(",");
}
function profileJob(job) {
  ...
  for (const step of job.steps) {
    ...
    if (step.run) {
      for (const [tc, pattern] of Object.entries(BUILD_PATTERNS)) {
...
var repeatedSetup = {
  ...
  check(ctx) {
    ...
    for (const profile of profiles.values()) {
      if (!profile.hasCheckout || profile.toolchains.size === 0 || !profile.builds)
        continue;
      if (profile.downloadsArtifact)
        continue;
      const key = [
...
// src/rules/no-path-filter.ts
function triggerHasPathFilter(trigger) {
  if (!trigger)
    return false;
  return Boolean(trigger.paths?.length || trigger["paths-ignore"]?.length);
}
function workflowHasPathFilters(wf) {
  const { triggers } = wf;
  if (triggerHasPathFilter(triggers.push))
    return true;
  if (triggerHasPathFilter(triggers.pull_request))
    return true;
  if (triggerHasPathFilter(triggers.pull_request_target))
    return true;
  return false;
}
var noPathFilter = {
  ...
  check(ctx) {
    ...
    if (!hasPushOrPR)
      return [];
    if (workflowHasPathFilters(ctx.workflow))
      return [];
    const othersHaveFilters = ctx.allWorkflows.some((wf) => wf.path !== ctx.workflow.path && workflowHasPathFilters(wf));
    if (!othersHaveFilters)
      return [];
    const triggerNames = [];
    if (triggers.push)
      triggerNames.push("push");
...
// src/rules/matrix-max-parallel.ts
var DEPLOY_PUBLISH = /\b(deploy|publish|release|push)\b/i;
function hasRateLimitedStep(job) {
  for (const step of job.steps) {
    if (step.run && DEPLOY_PUBLISH.test(step.run))
      return true;
    if (step.uses) {
      const { key } = parseActionRef(step.uses);
      if (DEPLOY_PUBLISH.test(key))
        return true;
    }
  }
  return false;
}
var matrixMaxParallel = {
  ...
  check(ctx) {
    const findings = [];
    for (const [jobId, job] of ctx.workflow.jobs) {
      if (!job.strategy?.matrix || job.strategy["max-parallel"] == null)
        continue;
      if (hasRateLimitedStep(job))
        continue;
      const label = job.name ?? jobId;
      findings.push({
...
// src/rules/unpinned-action.ts
var SHA_RE = /^[0-9a-f]{40}$/;
var VERSION_TAG_RE = /^v\d/;
var unpinnedAction = {
  id: "unpinned-action",
  tier: "static",
  severity: "info",
...
var flakyOrHanging = {
  ...
  check(ctx) {
    ...
    for (const [jobName, durations] of jobDurations) {
      ...
      if (med > 0 && p95 >= med * 3 && p95 - med >= 5 * 60000) {
  ...
  check(ctx) {
    ...
    for (const run of auditData.runs) {
      if (run.jobs.length === 0)
        continue;
      const jobStarts = run.jobs.map((j) => j.startedAt).filter((s) => s != null).map((s) => new Date(s).getTime());
      const jobEnds = run.jobs.map((j) => j.completedAt).filter((s) => s != null).map((s) => new Date(s).getTime());
      if (jobStarts.length === 0 || jobEnds.length === 0)
        continue;
...
// src/rules/double-run-measured.ts
function runDurationFromJobs(run) {
  let total = 0;
  for (const job of run.jobs) {
    const d = durationMs(job.startedAt, job.completedAt);
    if (d != null)
...
var doubleRunMeasured = {
  ...
  check(ctx) {
    ...
    for (const run of auditData.runs) {
      if (!PAIRABLE_EVENTS.has(run.event))
        continue;
    ...
    for (const [, runs] of groups) {
      ...
      findings.push({
        ...
        estimatedSavings: {
          confidence: "exact",
          minutesPerRun: shorterDuration > 0 ? Math.round(shorterDuration / 60000) : undefined
        }
      });
    }
    return findings;
...
`)) {
  ...
  for (const f of findings)
    counts[f.severity]++;
  lines.push("");
  lines.push(`${findings.length} finding(s): ${counts.high} high, ${counts.medium} medium, ${counts.low} low, ${counts.info} info`);
  return lines.join(`
`) + `
`;
}
...
function formatSarif(findings, rules) {
...
async function fetchJobsForRun(nwo, runId) {
  const data = await apiGet(`/repos/${nwo}/actions/runs/${runId}/jobs?per_page=100`);
  if (!data || typeof data !== "object")
    return [];
...
async function run() {
  ...
  const hasAboveThreshold = failOn !== "none" && findings.some((f) => SEVERITY_ORDER2[f.severity] <= SEVERITY_ORDER2[failOn]);
